void plot_carynbellows(int b = 70, int plot=0){
  ...
  TString filename = "_ftsdV2Uvirtual_track1_carynplots.root";
  TFile *_file0 = TFile::Open(Form("/volatile/halla/moller12gev/palatchi/remoll_bellows_beam_ftsdV2Uvirtual/det%d%s",b,filename.Data()));
  ...
  TCanvas *c0 = new TCanvas("c0","c0",100,100,700*1.5,700*1.5);  
    ...
    //get energy info
      h=(TH1D*)d->Get("hE");
      double Nbins=11000;//1000
      double Min=0;
      double Max=11000;//1500
      if(b==28){
	Nbins=11000;
	Max=11000;
      }
