void caryn_bellows_quickmatch(int detnum=0){
 ...
 TString outfilename = Form("/volatile/halla/moller12gev/palatchi/remoll_bellows_beam_ftsdV2Uvirtual/remoll_electrons_ftsdV2Uvirtual_quickmatch_det%d.txt",detnum);
 cout<<"writing output to "<<outfilename.Data()<<endl;
  ...
  if(runnum>99){
   ...
   //filter list: keep if e's hit bellows, check before and after ii for hit to 28 or 5547
	...
	cout<<"Ne bellows&28sum "<<Nebothsum<<endl;
	if(Neboth>0){
