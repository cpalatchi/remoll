void plot_carynquickmatch(int plot=0){
  ...
  TString filename = "count_ftsdV2Uvirtual_quickmatch.root";
  TFile *_file0 = TFile::Open(Form("/volatile/halla/moller12gev/palatchi/remoll_bellows_beam_ftsdV2Uvirtual/%s",filename.Data()));
  ...
  //Get energy #N info
	...
	cjj++;
      ...
      TCanvas *c2 = new TCanvas("c2","c2",100,100,1400*2,700*2);  
      c2->cd();
	...
	tree->Draw(Form("y:x",detnum),Form("track==1&&det==%d%s",detnum,cut.Data()),"*");
	c2->cd(cjj+1+Ndet);
	tree->Draw(Form("y5547:x5547",detnum),Form("track==1&&det==%d%s",detnum,cut.Data()),"*");
	c2->cd(cjj+1+Ndet*2);
	tree->Draw(Form("acos(x/r)*sign(y)",detnum),Form("track==1&&det==%d%s",detnum,cut.Data()));
	cjj++;
	}
      }

      TCanvas *c3 = new TCanvas("c3","c3",100,100,1400*2,700*2);  
      c3->cd();
      gStyle->SetOptStat("eMRoui");
      c3->Divide(Ndet,3);
      cjj=0;
	...
	tree->Draw(Form("acos(pz/p)*sign(pz)>>he_%dplotth",detnum),Form("track==1&&det==%d%s",detnum,cut.Data()));
	//	if(detnum==76){
	//          h=(TH1D*)_file0->FindObject(Form("he_%dplotth",detnum));
	//	  h->GetXaxis()->SetRangeUser(0,11000);
	//	}
	c3->cd(cjj+1)->SetLogy();
	c3->cd(cjj+1+Ndet);
	tree->Draw(Form("acos(pz5547/p5547)*sign(pz5547)>>he_%dplotth5547",detnum),Form("track==1&&det==%d%s",detnum,cut.Data()));
	//	if(detnum==76){
	//          h=(TH1D*)_file0->FindObject(Form("he_%dplotth5547",detnum));
	//	  h->GetXaxis()->SetRangeUser(0,11000);
	//	}
	c3->cd(cjj+1+Ndet)->SetLogy();
	c3->cd(cjj+1+Ndet*2);
	tree->Draw(Form("acos(pz/p)*sign(pz):e",detnum),Form("track==1&&det==%d%s",detnum,cut.Data()),"*");
	//	c3->cd(cjj+1+Ndet*2)->SetLogz();
	cjj++;
	}
      }
